 ...
 *         Sep 24, 2015
 */
public class TestNgHandlerUpload extends DefaultHandler {
  ...
  @Inject
  public TestNgHandlerUpload(ExecutorUploadService executorUploadService, @Assisted("functionalJobId") String functionalJobId) {
    this.executorUploadService = executorUploadService;
    KeywordUploadJob job = (KeywordUploadJob) this.executorUploadService
        .get(functionalJobId);
    Iterator<Map.Entry<String, String>> iterator = job.getRawDataOutput()
        .entrySet().iterator();
  ...
  public void startParsing() throws SAXException, IOException,
  ...
  public void startElement(String uri, String localName, String qName,
    ...
    if ("test-method".equalsIgnoreCase(qName)) {
      if (inclass) {
        if (isFirstMethod) {
          try {
            tmpSuiteReport.setRunningTime(new SimpleDateFormat(
                "yyyy-MM-dd'T'HH:mm:ssX").parse(attributes
                .getValue("started-at")));
        ...
        if ("tearDown".equalsIgnoreCase(attributes.getValue("name"))
          ...
          tmpSuiteReport
              .setTotalTestCase(tmpSuiteReport.getTotalTestCase() + 1);
