...
import org.ats.service.report.function.TestNgHandlerUpload;
 ...
 * Sep 24, 2015
 */
public class ReportUploadService extends AbstractMongoCRUD<Report>{
  
  private final String COL_NAME = "report";

  @Inject
  private ReportTestNgUploadFactory reportTestNgUploadFactory;
  
  @Inject
  private ReportJmeterFactory reportJmeterFactory;

  @Inject
  public ReportUploadService(MongoDBService mongoService, Logger logger) {
  ...
  public PageList<Report> getList(String jobId, Type jobType, String scriptId) throws Exception {
    ...
    if (jobType == Type.PERFORMANCE) {
      ...
      if (list.count() == 0) {
        TestNgHandlerUpload testNgHandlerUpload = reportTestNgUploadFactory.create(jobId);
        testNgHandlerUpload.startParsing();
        Map<String, SuiteReport> listSuiteReport = testNgHandlerUpload.getListSuiteReport();
        Report report = new Report();
