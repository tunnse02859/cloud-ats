...
public class Case extends AbstractTemplate {
  ...
  @Inject
  private Case(@Assisted("projectId") String projectId, @Assisted("name") String name, @Nullable @Assisted("dataDriven") DataDrivenReference ref, @Assisted("creator") String creator) {
    ...
    for (JsonNode json : actions) {
      if (json.has("steps")) {
        for (JsonNode step : json.get("steps")) {
          transform(step, sb, valueDelayTransform, actionPause);
        }
      } else {
        transform(json, sb, valueDelayTransform, actionPause);
      }
    }
  ...
  public static String buildStartStepLog(JsonNode json) {
    List<String> listParams = new ArrayList<String>();
    StringBuilder sb = new StringBuilder();
  ...
  private void transform(JsonNode json, StringBuilder sb, int valueDelayTransform, AbstractAction actionPause) throws IOException {
    ...
    if(!action.getAction().equals("loopor")){
      String str = Case.buildStartStepLog(json);
      sb.append(str);
    }
